...
window.addEventListener('keydown',e=>{if(dialog.open||e.metaKey||e.ctrlKey||e.altKey)return;if(keyMap[e.code]){const dir=keyMap[e.code];if(!keys.has(dir)){const step=dir==='forward'?.12:dir==='back'?-.12:0;const nx=avatar.position.x-Math.sin(yaw)*step,nz=avatar.position.z-Math.cos(yaw)*step;if(!blocked(nx,nz)){avatar.position.x=nx;avatar.position.z=nz;}if(dir==='left')yaw+=.035;if(dir==='right')yaw-=.035;}keys.add(dir);e.preventDefault();}if(e.code==='Space'){e.preventDefault();if(!e.repeat)doJump();}if((e.code==='Enter'||e.code==='NumpadEnter')&&!e.repeat&&nearby){e.preventDefault();openSection(nearby.id);}});window.addEventListener('keyup',e=>keys.delete(keyMap[e.code]));window.addEventListener('blur',()=>keys.clear());document.addEventListener('visibilitychange',()=>{keys.clear();previous=performance.now()});
...
camTarget.set(0,1.65,22);camera.lookAt(camTarget);renderer.render(scene,camera);$('#loading').classList.add('done');setTimeout(()=>$('#loading').hidden=true,700);toast('Welcome to the district. WASD to move · Drag to look · Enter to explore.');requestAnimationFrame(frame);
}
